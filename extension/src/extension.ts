  ...
  let startVoiceCommandDisposable = vscode.commands.registerCommand('voice-to-code.startVoiceCommand', async () => {
    ...
    if (!token) {
      const loginFirst = await vscode.window.showInformationMessage(
        'You need to login first to use Voice to Code',
        'Login'
      );
    ...
    try {
      ...
      if (transcription.includes('LLM')) {
        await handleLLMEnhancement(transcription, token as string);
      } else {
        ...
        // Log usage
        await logUsage(token as string, 'voice-to-code', transcription.length);
      }
    } catch (error) {
...
function processVoiceCommand(transcription: string): string {
...
async function handleLLMEnhancement(transcription: string, token: string) {
...
async function logUsage(token: string, feature: string, codeLength: number) {
  try {
    await axios.post(`${API_BASE_URL}/usage`, 
      { 
        feature,
        codeLength,
...
function getPythonPath(): string {
  ...
  function createSpeechToTextScript(scriptPath: string): void {
    const scriptContent = `
  import sys
  import os
  import time
